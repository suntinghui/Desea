        ...
        for (NSString *key in [tempDic allKeys]) {
            ...
            if ([obj isKindOfClass:[NSDictionary class]]) {
                [mutString appendFormat:@"<%@><![CDATA[%@]]></%@>", key, [XMLWriter XMLStringFromDictionary:obj], key];
                 ...
                 [mutString appendFormat:@"<%@><![CDATA[%@]]></%@>", key, obj, key];
                //表单请求的info字段 不需要加 <![CDATA[%@]]>
//                if ([[reqDic objectForKey:kMethodName] isEqualToString:@"SetGLBD"]&&[key isEqualToString:@"Infors"])
//                {
//                    [mutString appendFormat:@"<%@>%@</%@>", key, obj, key];
//                }
//                else
...
//                }
    ...
    [[Transfer sharedClient] setDefaultHeader:@"SOAPAction" value:[NSString stringWithFormat:@"http://oa.desea.cn/%@", [reqDic objectForKey:kMethodName]]];
...
//        NSString *path = [[NSBundle mainBundle]pathForResource:@"file" ofType:@"txt"];
//        NSString *str = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:nil];
//        NSString * respXML = [str gtm_stringByUnescapingFromHTML];
//        id obj = [self ParseXMLWithReqName:[reqDic objectForKey:kMethodName] xmlString:respXML];
//        success(obj);
//
//        
...
- (void)sortdict:(NSDictionary*)dict
{
