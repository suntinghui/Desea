...
{
    if (!_textView) {
        _textView = [[UITextView alloc]initWithFrame:CGRectMake(10, 5, 250, 34)];
    ...
    if ([self convertYToWindow:CGRectGetMaxY(self.originalFrame)]>=_keyboardRect.origin.y)
    {
        ...
        if (self.frame.origin.y== self.originalFrame.origin.y) {
            ...
            [UIView animateWithDuration:0.3
                             ...
                             animations:^{
                                 self.transform = CGAffineTransformMakeTranslation(0, -_keyboardRect.size.height+[self getHeighOfWindow]-CGRectGetMaxY(self.originalFrame)-20-44);
                             } completion:nil];
        }
        else
        {
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if ([self.delegate respondsToSelector:@selector(inputBarBenginEditing)])
    {
        [self.delegate inputBarBenginEditing];
    }
}
