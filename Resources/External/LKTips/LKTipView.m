...
- (void)adjust {
	...
	if (keepFit) {
        ...
        CGSize s = [text sizeWithFont:[UIFont boldSystemFontOfSize:14] 
					constrainedToSize:CGSizeMake(self.bounds.size.width-20,200) 
						lineBreakMode:UILineBreakModeWordWrap];
        messageRect.size = s;
        messageRect.origin.x = (self.frame.size.width - s.width)/2;
		messageRect.origin.y = (self.frame.size.height - s.height)/2;
    }
